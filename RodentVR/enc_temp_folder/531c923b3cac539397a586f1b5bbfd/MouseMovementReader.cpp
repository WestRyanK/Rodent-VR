...
MouseMovementReader::MouseMovementReader()
{
	...
	for (size_t i = 0; i < DeviceCount; i++)
	{
		if (RawInputDeviceList[i].dwType == RIM_TYPEMOUSE)
		{
			...
			Debug(10.0f, FColor::Red, FString(DeviceName));
			this->RegisterRawInputFromDeviceHandle((HWND)RawInputDeviceList[i].hDevice);
		}
	}
...
{
	Debug(10.0f, FColor::Red, FString(std::to_string(Message).c_str()));
	if (Message == WM_INPUT)// || Message == WM_MOUSEMOVE)
	{
...
void MouseMovementReader::SetCursorHandles(const wchar_t* MouseAName, const wchar_t* MouseBName)
{
	...
	for (size_t i = 0; i < DeviceCount; i++)
	{
		...
		if (_wcsicmp(MouseAName, DeviceName) == 0)
		{
			...
			this->RegisterRawInputFromDeviceHandle((HWND)this->MouseAId);
		}
		if (_wcsicmp(MouseBName, DeviceName) == 0)
		{
			this->MouseBId = RawInputDeviceList[i].hDevice;
			this->RegisterRawInputFromDeviceHandle((HWND)this->MouseBId);
		}
	}
