...
namespace RawInput
{
	public sealed class RawMouse
	{
		...
		public delegate void DeviceEventHandler(object sender, RawMouseInputEventArg e);
		...
		public void EnumerateDevices()
		{
			lock (_padLock)
			{
				...
				if (Win32.GetRawInputDeviceList(IntPtr.Zero, ref deviceCount, (uint)dwSize) == 0)
				{
					...
					Debug.WriteLine("EnumerateDevices() found {0} Keyboard(s)", NumberOfMice);
					return;
				}
			}
		...
		public void ProcessRawInput(IntPtr hdevice)
		{
			...
			int flags = _rawBuffer.data.mouse.usFlags;
			// TODO: Add buttons!
			...
			MouseEvent?.Invoke(this, new RawMouseInputEventArg(mouseEvent));
		}
	}
}
